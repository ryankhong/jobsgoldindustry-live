...
function workscout_add_new_member() {
    ...
    if($login_system == 'workscout') {
        if (isset( $_POST["workscout_register_check"] ) && wp_verify_nonce($_POST['workscout_register_nonce'], 'workscout-register-nonce')) {
            ...
            $recaptcha_status  = Kirki::get_option( 'workscout','pp_woo_recaptcha', false);
            if($recaptcha_status) {
            ...
            if($recaptcha_status) {
                if(  !empty($recaptcha) ) {
                    ...
                    if( !$responseData->success ) {
                        workscout_form_errors()->add('captcha_fail', __('Captcha fail! Please try again','workscout'));
                    }

                } else {
                    workscout_form_errors()->add('captcha_fail', __('You forgot about captcha','workscout'));
...
function workscout_ajax_login_init(){
    ...
    $recaptcha_status  = Kirki::get_option( 'workscout','pp_woo_recaptcha', false);
    if($recaptcha_status) {
        wp_register_script('workscout-ajax-login-script', get_template_directory_uri() . '/js/ajax-login-script-recaptcha.min.js', array('jquery') ); 
    } else {
...
function workscout_ajax_register(){
    ...
    if($recaptcha){
        if( isset($_POST['grecaptcha']) && !empty($_POST['grecaptcha']) ) {
                ...
                $responseData = json_decode($verifyResponse);
                write_log($responseData);
                if( !$responseData->success ) {
                     echo json_encode(array('loggedin'=>false, 'message'=>__('Captcha fail! Please try again','workscout'), 'type'=>"error notification " ));
                        die();    
                }

        } else {
            echo json_encode(array('loggedin'=>false, 'message'=>__('You forgot about captcha','workscout'), 'type'=>"error notification " ));
           die();
        }
    }
    $password = wp_generate_password();
...
function ws_validate_re_captcha_field( $username, $email, $validation_errors ) {
    ...
    if($recaptcha){
        if( isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response']) ) {
            ...
            write_log($responseData);
            if( !$responseData->success ) {
                $validation_errors->add( 'instructor_error', __( 'Captcha fail! Please try again', 'workscout' ) );
            }
        } else {
            $validation_errors->add( 'instructor_error', __( 'You forgot about captcha!', 'workscout' ) );
        }
    }
}

add_action( 'woocommerce_register_post', 'ws_validate_re_captcha_field', 10, 3 );
