		...
		<?php if ( $company_fields ) : ?>
			...
			<?php 
				foreach ( $company_fields as $key => $field ) :
					if( $key == 'company_name' ){
						$field['value'] = get_field('dc_company_name' , 'user_'.$currentUserID);
					}

					if( $key == 'company_website' ){
						$field['value'] = get_field('dc_company_website' , 'user_'.$currentUserID);
					}

					if( $key == 'company_tagline' ){
						$field['value'] = get_field('dc_company_tag_line' , 'user_'.$currentUserID);
					}

					if( $key == 'company_twitter' ){
						$field['value'] = get_field('dc_twitter_username' , 'user_'.$currentUserID);
					}

					if( $key == 'company_logo' ){
						$field['value'] = get_field('dc_company_logo' , 'user_'.$currentUserID);
					}
			?>
				<fieldset class="form fieldset-<?php echo esc_attr( $key ); ?>">
					<label for="<?php echo esc_attr( $key ); ?>"><?php echo $field['label'] . apply_filters( 'submit_job_form_required_label', $field['required'] ? '' : ' <small>' . esc_html__( '(optional)', 'workscout' ) . '</small>', $field ); ?></label>
