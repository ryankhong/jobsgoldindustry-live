...
get_header(); ?>
<style type="text/css">
	/*#titlebar h1 span.job-type.temporary, #titlebar h1 span.job-type.full-time, #titlebar h1 span.job-type.full-time{*/
				...
				<div class="company-info <?php echo ($logo_position == 'left') ? 'left-company-logo' : 'right-company-logo' ;?>" >
					<?php if(class_exists('Astoundify_Job_Manager_Companies')) { echo workscout_get_company_link(the_company_name('','',false)); } ?>
						<div class="company_logo">
							<?php ($logo_position == 'left') ? the_company_logo('full') : the_company_logo('full'); ?>
						</div>
					...
					<div class="content">
						...
						<?php if ( get_the_company_twitter() ) : ?>
							<span><a target="_blank" href="http://twitter.com/<?php echo get_the_company_twitter(); ?>">
		...
		<div class="widget">
			...
			<div class="job-overview">
				...
				<ul>
					...
					<?php if (in_array("job_salary", $overview_elements)) : ?>
					<?php $job_salary = get_post_meta( $post->ID, '_job_salary', true ); 
					 if ( $job_salary ) { ?>
				...
				<?php 
					if ( candidates_can_apply() ) {
						...
						if(!empty($external_apply)) {
							echo '<a class="button applyCountBtn" data-job="'.$post->ID.'" target="_blank" href="'.esc_url($external_apply).'">'.esc_html__( 'Apply for job', 'workscout' ).'</a>';
						} else {
							get_job_manager_template( 'job-application.php' ); 
						}
				?>
					<script>
						jQuery('#job_email.wpcf7-hidden').val('<?php echo get_post_meta( get_the_ID() , '_application' , true ); ?>');
						jQuery('#job_title.wpcf7-hidden').val('<?php echo get_the_title(); ?>');
					</script>
				<?php } else{ ?>
					<p style="text-align: center; background: #C8BA7D; color: white; padding: 10px;">This position has already been filled</p>
				<?php } ?>				
