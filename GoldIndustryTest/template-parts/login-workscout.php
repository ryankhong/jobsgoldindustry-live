...
<ul class="float-right <?php echo (is_user_logged_in()) ? 'userLogged' : '' ; ?>">
	...
	if ( is_user_logged_in() ) { 
		if( ! empty( $custom_userpage )) { 
			...
			if($user_page_status){	?>
			<li>
				<a class="logout" href="<?php echo esc_url(apply_filters('workscout_userpage', $userpage_link)); ?>"><i class="fa fa-sign-out"></i> <?php esc_html_e('User Page','workscout') ?></a>
			</li>
		<?php 
			}
		} ?>
	...
	if($login_popup) { ?>
		<div id="signup-dialog" class="small-dialog workscout-way zoom-anim-dialog mfp-hide apply-popup workscout-signup-popup">
			<div class="small-dialog-headline">
				<h2><?php esc_html_e('Job Seeker Sign up','workscout'); ?></h2>
			</div>
